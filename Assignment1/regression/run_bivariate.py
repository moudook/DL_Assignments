...
from data_loader import load_bivariate_data
...
from shared.utils.plotting import (
    plot_error_curves,
    plot_regression_bivariate,
    plot_scatter_target_vs_output,
)

DATA_DIR = "data/Group11/Regression/BivariateData/11.csv"
X, y = load_bivariate_data(DATA_DIR)
...
# ── Train ──────────────────────────────────────────────────────────────────
model = Perceptron(n_features=2, activation='linear')
...
plot_error_curves(histories,
                  title="Bivariate Regression — Error vs Epochs",
                  save_path="outputs/regression/bivariate/error_curve.png")

# ── Plot 3: Model output superimposed on target ────────────────────────────
plot_regression_bivariate(model, X_train, y_train,
                            title="Bivariate — Train: Model vs Target",
                            save_path="outputs/regression/bivariate/model_vs_target_train.png")

plot_regression_bivariate(model, X_test, y_test,
                            title="Bivariate — Test: Model vs Target",
                            save_path="outputs/regression/bivariate/model_vs_target_test.png")

# ── Plot 4: Scatter — target (x) vs model output (y) ──────────────────────
plot_scatter_target_vs_output(y_train, train_pred,
                               title="Bivariate — Train: Target vs Output",
                               save_path="outputs/regression/bivariate/scatter_train.png")

plot_scatter_target_vs_output(y_test, test_pred,
                               title="Bivariate — Test: Target vs Output",
                               save_path="outputs/regression/bivariate/scatter_test.png")